...
##
##
## [3]  Create plot3.png - Evolution of the 3 energy subsystems metering vs.time
##
...
##      use type = "n" to start with empty plot
plot(x = power$day.time, y = power$Global_active_power, col = "black", ylab = "Energy sub metering", xlab = " ", type = "n", cex.lab = 0.98, cex.axis = 0.98, cex.main = 1.15, ylim = c(0,38))

##      add the three sub_metering
with(power, lines(day.time, Sub_metering_1))
with(power, lines(day.time, Sub_metering_2, col = "red"))
with(power, lines(day.time, Sub_metering_3, col = "blue"))

##      add the legend
legend("topright", col = c("black", "blue", "red"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=c(1,1), cex = 0.95)
